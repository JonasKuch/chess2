...
class TensorProcessor:
    def __init__(self):
        ...
        self.white_move_map = {}
        self.black_move_map = {}
        self.rev_white_move_map = {}
        self.rev_black_move_map = {}

        for idx, m in enumerate(self.MOVES):
            self.white_move_map[m] = idx
            self.rev_white_move_map[idx] = m
            m_black = m.translate(str.maketrans("12345678", "87654321"))
            self.black_move_map[m_black] = idx
            self.rev_black_move_map[idx] = m_black
    ...
    def fen_to_tensor(self, fen):
        ...
        flags = np.array([[0, 0, 0, 0, 0]])
        '''
        Own Pawns (occuppied fields marked as 1, others as 0)
        Own Knights
        Own Bishops
        Own Rooks
        Own King
        ...
        piece_map_white = {"P":0, "N":1, "B":2, "R":3, "K":4, "Q":5,
                    "p":6, "n":7, "b":8, "r":9, "k":10, "q":11}
        piece_map_black = {"p":0, "n":1, "b":2, "r":3, "k":4, "q":5,
                    "P":6, "N":7, "B":8, "R":9, "K":10, "Q":11}
    ...
    def uci_to_idx(self, best_move, side_to_move):
        if side_to_move == "w":
            idx = self.white_move_map[best_move]
        elif side_to_move == "b":
            idx = self.black_move_map[best_move]
        return idx


    def index_to_uci(self, idx: int, side_to_move) -> str:
        if side_to_move == "w" or side_to_move == Color.WHITE:
            move = self.rev_white_move_map[idx]
        elif side_to_move == "b" or side_to_move == Color.BLACK:
            move = self.rev_black_move_map[idx]
        return move
        

    def decode_policy_vector(self, vec, side_to_move):
        """
        Given a length-1858 policy vector (logits or probs or one-hot),
