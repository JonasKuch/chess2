...
new_white_move_map = {}
new_black_move_map = {}
new_rev_white_move_map = {}
new_rev_black_move_map = {}

for idx, m in enumerate(MOVES):
    new_white_move_map[m] = idx
    new_rev_white_move_map[idx] = m
    m_black = m.translate(str.maketrans("12345678", "87654321"))
    new_black_move_map[m_black] = idx
    new_rev_black_move_map[idx] = m_black

# -----------------------------------------------------------------------------
# 1) Define the V3 record struct
# -----------------------------------------------------------------------------
# C++ struct V3TrainingData is packed, little‑endian:
#   uint32_t version;
...
#   int8_t  result;
#
# Total size = 8276 bytes
V3_STRUCT = struct.Struct('<I'      # version
                          '1858f'   # probabilities
                          '104Q'    # bitboard planes
                          '7B'      # 4 castling + side + rule50 + move_count
                          'b')      # result

RECORD_SIZE = V3_STRUCT.size  # 8276

# -----------------------------------------------------------------------------
# 2) A generator that yields decoded records from a .gz file object
# -----------------------------------------------------------------------------
def parse_v3_gzip_stream(gzip_stream):
    """
    ...
    with gzip.GzipFile(fileobj=gzip_stream, mode='rb') as f:
        while True:
            ...
            yield {
                ...
                'probabilities': probs,
                'planes': planes,
                'castling_us_ooo': cast_us_ooo,
                'castling_us_oo':  cast_us_oo,
                'castling_them_ooo':cast_th_ooo,
                'castling_them_oo': cast_th_oo,
                'side_to_move': side,
                'rule50_count': rule50,
                'move_count': mv_cnt,
                'result': result
...
# -----------------------------------------------------------------------------
# 3) Iterate through the tar.bz2, find .gz members, parse each record
# -----------------------------------------------------------------------------
def iterate_ccrl_v3(tar_bz2_path):
    """
    Stream through `tar_bz2_path`, for each .gz file inside, yield parsed records.
    """
...
def bitboard_to_matrix(bb):
    """
    bb: a Python integer 0 <= bb < 2**64
    returns: 8×8 numpy array, row‑major with row 0 = rank 1, col 0 = file a
    """
    # unpack into 64 bits, LSB first
    bits = np.unpackbits(
        np.array([bb], dtype=np.uint64).view(np.uint8)
    )
    # bits now length‑64, but in little‑endian byte order → reverse to get LSB-first bit order
    bits = np.flip(bits.reshape(8,8), axis=(0,1))
    # reshape into ranks/files:
    return bits  # shape (8 ranks, 8 files)
...
if __name__ == '__main__':
    ...
    # grab just the first record
        ...
        print("  # probabilities:", first['probabilities'].shape)
        print("  # bitboard planes:", first['planes'].shape)
        print("  castling rights (us_ooo, us_oo, them_ooo, them_oo):",
            first['castling_us_ooo'],
            first['castling_us_oo'],
            first['castling_them_ooo'],
            first['castling_them_oo'])
        print("  side to move:", first['side_to_move'])
        print("  rule50 count:", first['rule50_count'])
        print("  move count:", first['move_count'])
...
'''
    The 104 boards consist of the 8*12 boards that represent the last 8 moves (piecewise)
    what my code here does is print the board always in the perspective of the player to move and interpret the probs vector as move
    i can exactly use this type of code to generate my training data

    the informations are correct sice they come from:
    https://github.com/LeelaChessZero/lczero-training/blob/master/tf/decode_training.py
